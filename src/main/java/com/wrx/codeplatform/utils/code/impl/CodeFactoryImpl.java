...
public class CodeFactoryImpl implements CodeFactory {
    ...
    private void classRechecking(String model, int targetClassId, int targetCodeId){
        ...
        recheck(model, recheckingResult, targetCodeId, codeList);
    }
     ...
     *
     * @param model        模式
     * @param targetCodeId   目标代码
     */
    private void systemRechecking(String model, int targetCodeId){
        List<Code> codeList = ServiceFactory.codeService.selectAllCodes();
        RecheckingResultInit recheckingResult = new RecheckingResultInit(-99, targetCodeId, model, RANGE_SYSTEM);
        recheck(model, recheckingResult, targetCodeId, codeList);
    }
     ...
     *
     * @param model             模式
     * @param recheckingResult  查重结果实体
     * @param targetCodeId      目标代码
     */
    private void recheck(String model,RecheckingResultInit recheckingResult, int targetCodeId, List<Code> codeList){
        ...
        System.out.println("作业长度: "+codeList.size());
    ...
    private List<String> slotMapByValue(Map<Integer, Double> maps){
        ...
        for (Map.Entry<Integer, Double> integerDoubleEntry : list) {
            double per = integerDoubleEntry.getValue()<=0.7?integerDoubleEntry.getValue()/3.0:integerDoubleEntry.getValue();
            per = per<0?0:per;
            BigDecimal bg = new BigDecimal(per);
            per = bg.setScale(2, RoundingMode.HALF_UP).doubleValue();
            result.add(integerDoubleEntry.getKey() + "::" + per);
            if (result.size() >= 20){
                break;
            }
        }
